...
def gen_train_test(root_dir, test_size=0.2, seed=2022, distance_weighting=False):
    ...
    for dir_name, files in [("test", test_files), ("train", train_files)]:
        for tile in tile_types:
            ...
            target_dir = BaseClass.create_if_not_exists(root_dir / BaseClass.DIR_STRUCTURE[f"{dir_name}_{tile}s"], overwrite=True)
            ...
            for file_path in files:
                if tile == "image":
                    ...
                    if source_path.exists():
                        ...
                        raise IOError(f"Couldn't find corresponding {tile} file for {file_path.name}.")
