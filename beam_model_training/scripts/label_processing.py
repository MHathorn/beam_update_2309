...
- The script logs its progress and any warnings or errors encountered during execution.
"""
...
def get_image_names(json_data):
    ...
    Parameters:
    - json_data (list): An export of LabelStudio tasks, each containing labels for an image.

    Returns:
    - list: A list of image names extracted from the tasks.
    """
    input_images = []
...
def get_gdf_from_result(result, img, crs):
    ...
    gdf = gdf.to_crs(crs) # Convert to desired CRS only after pixel-to-coords transformation
    return gdf
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Process JSON files exported from LabelStudio into a shapefile of labels.')
    parser.add_argument('--json_dir', type=str, required=True, help='Directory containing JSON files')
    parser.add_argument('--img_dir', type=str, required=True, help='Directory where TIFF images used for labeling will be found')
    parser.add_argument('--shp_dir', type=str, required=True, help='Directory where shapefiles will be saved')
    ...
    if file_gdfs:
        ...
        try:
            gdf.drop(columns=["image_bounds"]).to_file(shp_dir / "labeled_buildings.shp")
